...
def writeInternalNetworkAttributes(response, lines):
    ...
    line = str(originId) + ',' + str(ipAddress) + ',' + str(prefixLength) + ',' + siteName + ',' + createdAt + ',' + name + ',' + str(siteId) + '\n'
    lines.append(line)
    return lines

def create_internal_networks(token_type):

    logfile = getPath("LOGFILES") + 'CREATE_INTERNAL_NETWORKS' + str(timestamp) + ".csv"
    ...
    with open(str(logfile), 'w', encoding='utf-8') as logFile:
        ...
        for network in networks:
            ...
            print(colored('Adding Internal Network: ' + network['name'],'green'))
